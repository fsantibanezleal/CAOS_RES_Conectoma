...
Adopted from the account's template, with one extension: a gate in a TypeScript or JavaScript file is
checked by the string its check is labelled with (a vitest case's title, a browser gate's label), since
those files have no ``def``. Without it, a front-end requirement could only name a whole file.
...
#: The rest of the line: a Python gate is one token, a front-end gate is a check's title, which has
#: spaces in it (a vitest case, a labelled browser check).
...
def gate_exists(root: Path, gate: str) -> tuple[bool, str]:
    ...
    if test_part:
        ...
        if path.suffix == ".py":
            ...
            # A TypeScript or JavaScript gate has no `def`: a vitest case is named by its string, and a
            # browser gate labels each of its checks with one. The name must appear as that string, so a
            # requirement can point at the ONE check that fails when it is violated, not at a whole file.
            quoted = (f"'{test_part}'", f'"{test_part}"', f"`{test_part}`")
            if not any(label in content for label in quoted):
                return False, f"names {test_part!r}, which no check in {path_part} is labelled with"
    return True, ""
