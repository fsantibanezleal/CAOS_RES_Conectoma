...
export default function Implementation() {
    ...
    <section>
      ...
      <Commands
        lines={[
          ...
          'python data-pipeline/run.py fetch-vision --source tartanair   # 2,244 clips, 114.5 GB, member by member',
          'python data-pipeline/run.py fetch-vision --source sintel      # the engine downloads and renders Sintel',
          'python data-pipeline/run.py render-vision --source tartanair  # every clip onto the lattice (contract 1)',
          'python data-pipeline/run.py build-splits                      # geometry families and the leakage gate',
          'python data-pipeline/run.py fetch-vision --source panorama    # the panoramas of the pure-rotation case',
          'python data-pipeline/run.py build-cases                       # sixteen cases at six levels',
          'python data-pipeline/run.py export-eyeclips                   # the eye input artifact and its manifest',
        ]}
      />
      <p>
        {t(
          'The vision lane fetches only the members of the remote archives a clip needs, over HTTP ranges, each checked against its CRC32 and written as one archive per clip; a log makes every fetch resumable. Rendering, the splits and the cases run in parallel processes and keep every finished clip, keyed on the content it depends on (the configuration, the render version and a digest of the rendering code), so a rerun renders only what changed. Long runs go through the operating system’s scheduler with no console window, because a job started from a shell dies with it.',
          'La línea de visión descarga solo los miembros de los archivos remotos que un clip necesita, por rangos HTTP, cada uno verificado contra su CRC32 y escrito como un archivo por clip; un registro hace reanudable cada descarga. El renderizado, las particiones y los casos corren en procesos paralelos y conservan cada clip terminado, con una clave sobre el contenido del que dependen (la configuración, la versión del renderizado y un digest del código que renderiza), así que una nueva ejecución renderiza solo lo que cambió. Las ejecuciones largas pasan por el planificador del sistema operativo sin ventana de consola, porque un trabajo lanzado desde una terminal muere con ella.',
        )}
      </p>
      <p>
        {t(
  ...
  const contracts = (
    <section>
      ...
      <p>
        {t(
          'The vision contract checks every clip rendered onto the lattice before anything reads it: each source declares the arrays it must carry, shapes must agree, luminance must lie in [0, 1], depth must be positive or NaN (masked, counted, never dropped), shares must lie in [0, 1], poses must be unit quaternions and video frames consecutive; a clip that breaks it is rejected with its reasons. The eye input artifact is one compact file per case, every array quantised to bytes, with a manifest giving each file’s size and digest and every column’s sampling position; the web validates each file against a TypeScript mirror and its digest before drawing it.',
          'El contrato de visión verifica cada clip renderizado sobre la retícula antes de que algo lo lea: cada fuente declara los arreglos que debe traer, las formas deben coincidir, la luminancia debe estar en [0, 1], la profundidad debe ser positiva o NaN (enmascarada, contada, nunca descartada), las fracciones deben estar en [0, 1], las poses deben ser cuaterniones unitarios y los cuadros de video consecutivos; un clip que lo rompe se rechaza con sus razones. El artefacto de la entrada del ojo es un archivo compacto por caso, cada arreglo cuantizado a bytes, con un manifiesto que da el tamaño y el digest de cada archivo y la posición de muestreo de cada columna; la web valida cada archivo contra un espejo TypeScript y su digest antes de dibujarlo.',
        )}
      </p>
      <SectionRefs ids={['eckstein2024', 'berg2026', 'lappalainen2024']} />
    </section>
  );
  ...
  const gates = (
    <section>
      ...
      <p>
        {t(
          'The suite runs in continuous integration on every push: the build contract, the column inference and its holdout, the comparison and its orientation measure, the placement rule, the compiler against the engine’s own on the published connectome, the target-centric totals, the regimes and their gradients, the size-matched controls, the neuron-level classes and the loop-gain estimate; for the vision lane, the member fetch against a local range server, the renderer against the engine’s box eye, each rule of the vision contract, the leakage test, every physical variant against its unit, the exactness of the synthetic and rotation cases, and the case summary against the registry and the case pages. Tests that need the local release cache, the downloaded ensemble, the engine’s Sintel rendering or an OpenGL context skip in CI and run locally.',
          'La suite corre en integración continua en cada push: el contrato de construcción, la inferencia de columnas y su exclusión, la comparación y su medida de orientación, la regla de ubicación, el compilador contra el del motor sobre el conectoma publicado, los totales centrados en el destino, los regímenes y sus gradientes, los controles igualados en tamaño, las clases a nivel de neurona y la estimación de ganancia de lazo; para la línea de visión, la descarga de miembros contra un servidor local de rangos, el renderizador contra el ojo de caja del motor, cada regla del contrato de visión, la prueba de fuga, cada variante física contra su unidad, la exactitud de los casos sintéticos y de rotación, y el resumen de casos contra el registro y las páginas de casos. Las pruebas que necesitan la caché local de la liberación, el ensamble descargado, el renderizado de Sintel del motor o un contexto OpenGL se omiten en CI y corren localmente.',
        )}
      </p>
      <p>
        {t(
          'The web has its own gate, run on the built site in a real browser at 1280x800, 1600x900 and 2560x1440, in both themes and both languages. It first proves it is looking at this product: the brand reads Conectoma and the explorer artifact verified against its manifest. Then, on the App, the page is exactly the viewport, the control rail shows every control without scrolling, every tab bar is one row, and what is actually drawn (the union of the painted hexagons and the chart’s plot, not the size of the svg element) covers at least half the screen. Every documentation route must answer 200 with its own heading, take the full width, and, on Experiments and Benchmark, fill its tables from the reports. The explorer must verify its artifact even with WebCrypto removed, which is what a page served over plain HTTP gets. Each architecture diagram must be the one its tab names, in the reader’s language and drawn in the theme’s colours. The eye input is held to the same floors on each of its views, every lattice canvas must hold a picture (colours sampled inside the box it declares it painted), and every one of the sixteen cases must load a clip verified against its manifest. The deploy runs the same gate and uploads nothing that fails it.',
          'La web tiene su propia compuerta, corrida sobre el sitio construido en un navegador real en 1280x800, 1600x900 y 2560x1440, en ambos temas y ambos idiomas. Primero prueba que mira este producto: la marca dice Conectoma y el artefacto del explorador se verificó contra su manifiesto. Luego, en la App, la página es exactamente la ventana, el riel de controles muestra cada control sin desplazarse, cada barra de pestañas ocupa una fila, y lo que efectivamente se dibuja (la unión de los hexágonos pintados y el área del gráfico, no el tamaño del elemento svg) cubre al menos la mitad de la pantalla. Cada ruta de documentación debe responder 200 con su propio título, ocupar todo el ancho y, en Experimentos y Benchmark, llenar sus tablas desde los reportes. El explorador debe verificar su artefacto incluso sin WebCrypto, que es lo que recibe una página servida por HTTP simple. Cada diagrama de arquitectura debe ser el que nombra su pestaña, en el idioma del lector y dibujado con los colores del tema. La entrada del ojo cumple los mismos pisos en cada una de sus vistas, cada lienzo de retícula debe contener una imagen (colores muestreados dentro del recuadro que declara haber pintado), y cada uno de los dieciséis casos debe cargar un clip verificado contra su manifiesto. El despliegue corre la misma compuerta y no sube nada que la falle.',
        )}
      </p>
